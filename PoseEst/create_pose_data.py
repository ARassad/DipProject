...
def process (input_image, params, model_params):
    ...
    multiplier = [x * model_params['boxsize'] / input_image.shape[0] for x in scale_search]
    ...
    canvas = np.zeros((input_image.shape[0], input_image.shape[1], input_image.shape[2]), dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    if not os.path.exists("A:\\Diplom\\Project\\data\\source"):
        os.makedirs("A:\\Diplom\\Project\\data\\source")
    if not os.path.exists("A:\\Diplom\\Project\\data\\pose"):
        os.makedirs("A:\\Diplom\\Project\\data\\pose")
    ...
    while(cam.isOpened()) and ret_val == True and i < ending_frame:
        if i % frame_rate_ratio == 0:
            ...
            pose = process(input_image, params, model_params)

            print('Processing frame: ', i)
            toc = time.time()
            print('processing time is %.5f' % (toc - tic))

            Image.fromarray(pose).save("A:\\Diplom\\Project\\data\\pose\\{}.png".format(i))
            Image.fromarray(input_image).save("A:\\Diplom\\Project\\data\\source\\{}.png".format(i))
